...
public class MapManager : MonoBehaviour
{
    ...
    private EntityManager m_CurrentManger;
    ...
    public void Awake()
    {
        ...
        m_CurrentManger = FindObjectOfType<EntityManager>();
        
    }
    private void Update()
    {
        if (m_CurrentManger.outpostIA == null)
        {
            SetAlignementZone(Alignment.Player, new Vector3(0, 0, mapData.height / 2 + 1), mapData.width/2, mapData.height / 2);
        }
        if (m_CurrentManger.outpost2IA == null)
        {
            SetAlignementZone(Alignment.Player, new Vector3(mapData.width / 2, 0, mapData.height / 2 + 1), mapData.width/2, mapData.height/2);
        }
        if (m_CurrentManger.outspotPlayer == null)
        {
            SetAlignementZone(Alignment.IA, Vector3.zero, mapData.width/2, mapData.height / 2);
        }
        if (m_CurrentManger.outspotPlayer2 == null)
        {
            SetAlignementZone(Alignment.IA, new Vector3(mapData.width / 2, 0, 0), mapData.width, mapData.height/2);
        }
    }
